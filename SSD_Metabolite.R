...
data <- read_excel("SepticShockDataR.xlsx", sheet = "Metabolite Data") %>%
...
# Select relevant columns starting from column 6
metabolite_data <- data[,-1:-5]

# Extract column names for clinical observations
observation_columns <- names(metabolite_data)
...
write.csv(summary_table, "SSD_Metabolite_sig_observations.csv", row.names = FALSE)

# Save significant ANOVA results to a CSV file
write.csv(significant_observations, "SSD_Metabolite_anova_sig_results.csv", row.names = FALSE)

# Save significant Tukey results to a CSV file
write.csv(tukey_significant_results, "SSD_Metabolite_tukey_sig_results.csv", row.names = FALSE)