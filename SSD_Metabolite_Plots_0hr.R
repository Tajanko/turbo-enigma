...
# Filter to retain only rows where timepoint == 0
data <- data %>%
  filter(Timepoint == 0)
...
write.csv(tidy_tukey, "SSD_Metabolite0hr_TukeyHSD.csv", row.names = FALSE) 
...
write.csv(sig_observations, "SSD_Metabolite0hr_Sig_Observations.csv", row.names = FALSE)
...
for (observation in sig_observations$Observation) {
  ...
  p <- ggplot(plot_data, 
    ...
    geom_point(aes(fill = Sex), 
               ...
               show.legend = F) +
    labs(title = paste("Results for", original_observation, "at baseline"),
  ...
  ggsave(filename = paste0(observation, 
                           "_0hr.png"), 
         path = '~/R/SepticShock/Plots/Metabolite/0hr',
         plot = p, 
         width = 8, 
         height = 6)
}
